...
function SpeakerRoom({
  ...
  // Hooks to join the channel, publish local tracks, and get remote users
  ...
  const handleRecording = async (action: "start" | "stop") => {
    ...
    try {
      // **FIX**: Pass the `channel` from connectionData in the request body
      const response = await fetch("/api/agora/recording", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action, channelName: channel }),
      });
      ...
      if (!response.ok || data.success === false) {
        throw new Error(
          data.error || data.message || `Failed to ${action} recording.`
        );
      }
    ...
    <div className="flex flex-col h-full bg-gray-900">
      <div className="flex-grow w-full h-3/5 p-4 relative">
        <div className="w-full h-full bg-black rounded-lg overflow-hidden border-2 border-blue-500">
          ...
          {isRecording && (
            <div className="absolute top-2 right-2 z-10 flex items-center gap-2 bg-red-600 text-white px-3 py-1 rounded-md animate-pulse">
      ...
      <div className="flex justify-center items-center flex-wrap gap-4 p-4 bg-gray-900 border-t border-gray-700">
        {!isRecording ? (
          <button
            ...
            className="px-4 py-2 rounded-lg font-semibold bg-blue-600 hover:bg-blue-500 disabled:bg-blue-900 disabled:cursor-not-allowed transition-colors">
          ...
          <button
            ...
            disabled={isProcessingRecording}
            className="px-4 py-2 rounded-lg font-semibold bg-red-600 hover:bg-red-500 disabled:bg-red-900 disabled:cursor-not-allowed transition-colors">
...
export default function SpeakPage() {
  ...
  // It's recommended to create the client only once.
  ...
  useEffect(() => {
    const joinCall = async () => {
      try {
        setLoading(true);
        setError(null);
        // Call our new API route to get the token
